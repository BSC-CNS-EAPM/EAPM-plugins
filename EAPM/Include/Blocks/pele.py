...
<<<<<<< HEAD:EAPM/Include/Blocks/PeleEAPM.py

=======
>>>>>>> main:EAPM/Include/Blocks/pele.py
...
boxCentersVariable = VariableList(
    ...
    category="PELE",
    prototypes=[modelVariable, ligandVariable, chainVariable, residueVariable, atomNameVariable],
)
...
def pele_action(block: SlurmBlock):
    ...
    boxCentersValue = block.variables.get("box_centers", None)
    ...
    if energyByResidueTypeValue not in energy_by_residue_types:
        raise ValueError(
            "%s not found. Try: %s" % (energyByResidueTypeValue, energy_by_residue_types)
        )
    import random
    ...
    # Parse box_centers
    if not isinstance(boxCentersValue, type(None)) or boxCentersValue is not None:
        box_centers = {}
