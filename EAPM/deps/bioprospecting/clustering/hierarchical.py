...
class cluster:
    ...
    def plotDendrogram(self, labels=None, dpi=100, label_rotation=45, figsize=None,
                       xticks_fontsize=10):
        """
        Plot dendogram containing the clustering information
        ...
        if labels != None:
            ...
            # p = p-(p[1]-p[0])
            # print(p)
            l = plt.xticks(p, self.ordered_labels, rotation=label_rotation, fontsize=xticks_fontsize)
            # l = plt.xticks(plt.xticks()[0], self.ordered_labels, rotation=label_rotation)
